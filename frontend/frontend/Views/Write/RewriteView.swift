...
//  @State var mapchoose: Bool = false
  ...
  @State var selectedToggle: Int
  // 0: 다이어리 / 1: 분양 / 2: 길냥이
  
//  @State private var title = ""
  
  
  @State var catMood: DropdownMenuOption?
      ...
      Button(action: {
        ...
        if selectedToggle == 0 {
          if let unwrapedMood = catMood {
            SendAPI.feedPUT(kind: kind, ID: id, emotion: catMood!.key,content: describe){
              print(catMood!.key)
              completion()
              completion()
            }
...
struct RewriteView_Previews: PreviewProvider {
    static var previews: some View {
      RewriteView(id: 0, kind: "diary",presented: .constant(true), selectedToggle: 0, catMood: DropdownMenuOption.singleCatMood, describe: "dd", completion: {})
    }
}
